...
p <- ggplot(dfs, aes(x = Organisaation.tyyppi, y = Costs/1e6)) + geom_bar(stat = "identity") + ggtitle("Costs by organization type") + coord_flip() + xlab("") + ylab("Total Costs (MEUR)")
print(p)

dfs = df %>% group_by(Organization) %>% summarise(Costs = sum(Price)/1e6) %>% arrange(desc(Costs))
dfss = dfs[1:20, ]; dfss = dfss[nrow(dfss):1, ]
dfs$Organization = factor(dfs$Organization, levels = rev(as.character(dfs$Organization)))
p <- ggplot(dfs, aes(x = Organization, y = Costs/1e6)) + geom_bar(stat = "identity") + ggtitle("Costs by organization") + coord_flip() + xlab("") + ylab("Total Costs (MEUR)")
print(p)

names(dfs) = c("Organization", "Total (EUR)")
...
p <- ggplot(dfs, aes(x = Publisher, y = Costs/1e6)) + geom_bar(stat = "identity") + ggtitle("Costs by publisher") + coord_flip() + xlab("") + ylab("Total Costs (MEUR)")
print(p)

names(dfs) = c("Publisher", "Total (EUR)")
...
p <- ggplot(dfs, aes(x = Material, y = Costs/1e6)) + geom_bar(stat = "identity") + ggtitle("Costs by material") + coord_flip() + xlab("") + ylab("Total Costs (MEUR)")
print(p)

names(dfs) = c("Material", "Total (EUR)")
...
p <- ggplot(dfs, aes(x = Type, y = Costs/1e6)) + geom_bar(stat = "identity") + ggtitle("Costs by publication type") + coord_flip() + xlab("") + ylab("Total Costs (MEUR)")
print(p)

names(dfs) = c("Type", "Total (EUR)")
write.csv(dfs, file = "table/cost_by_type.csv", quote = F, row.names = F)


dfs = df %>% group_by(Year) %>% summarise(Costs = sum(Price)/1e6) %>% arrange(Year)
dfs = dfs[nrow(dfs):1, ]
names(dfs) = c("Year", "Total (EUR)")
