...
p <- ggplot(dfs[1:20,], aes(x = Publisher, y = n)) + geom_bar(stat = "identity", fill = "lightgray", color = "black") + ggtitle("Entries by publisher") + coord_flip() + xlab("") + ylab("Entries (n)") + geom_text(aes(x=Publisher, y=32, label=paste(Percentage, "%", sep = ""), hjust=ifelse(sign(Percentage)>0, 0, 0)), position = position_dodge(width=1), size = 7)
print(p)
...
The total costs paid to scientific publishers by Finland increase roughly 10% per year (annual increase is indicated in the left figure). The top-10 publishers (out of `r length(unique(df$Publisher))`) correspond to `r round(100 * sum(dfs[["Costs"]][1:10])/sum(dfs[["Costs"]]))`% of the overall costs.


```{r foi-costbytime, message=FALSE, echo=FALSE, fig.width=10, fig.height=5}
...
library(cowplot)
p <- plot_grid(p1, p2, labels = c('B', 'C'), align = 'h', rel_widths = c(1, 1.3))
```
