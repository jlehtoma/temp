...
```{r foi-plots, message=FALSE, eval=TRUE, echo=FALSE, fig.show="hold", out.width="200px"}
...
p <- ggplot(dfs, aes(x = Vuosi, y = Hinta)) +
     ...
     label = paste(round(100 * diff(dfs$Hinta)/dfs$Hinta[-length(dfs$Hinta)], 1), "p", sep = ""), size = 8) + 
       ylab("Hinta (EUR)")
print(p)
