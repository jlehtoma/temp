...
dfs$Percentage = round(100 * dfs$Total/sum(dfs$Total), 2)
dfs[, as.character(c(2010:2015))] = round(dfs[, as.character(c(2010:2015))], 2)
dfs = rbind(dfs, c("Total", round(colSums(dfs[, c(as.character(c(2010:2015)), "Total")]), 2)))
dfs = dfs %>% arrange(desc(Percentage))
dfs$Total = round(as.numeric(dfs$Total), 1)
names(dfs) = c("Organization (costs in MEUR)", as.character(2010:2015), "Total", "Percent (%)")
...
```{r foi-timebypublisher2, message=FALSE, echo=FALSE, fig.width=12, fig.heigth=4}
